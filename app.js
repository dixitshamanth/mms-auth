...
app.post('/imageupload', upload.single('image'), async (req, res, next) => {
  ...
  await sharp('uploads/' + req.file.filename).resize(150, 150)
    .png({ quality: 100 }).toFile('uploads/' + req.file.filename + '-thumb');

  Member.findOneAndUpdate({ sabhe_id: req.body.sabhe_id },
    {
      img: {
        data: fs.readFileSync(path.join('uploads/' + req.file.filename + '-thumb')),
    ...
    , function (err) {
      if (err) {
        ...
        fs.unlink('uploads/' + req.file.filename + '-thumb', (err) => {
          if (err) {
            console.error(err.message)
            return
          }
